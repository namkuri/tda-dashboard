...
- **B3 게임 정전(Canon) 위키**: 사이드바 "🎮 게임 정전" 뷰 신설. 프로젝트별 canon 문서(조합/게임루프 등) — 카테고리 그룹 목록 + 에디터(제목/카테고리/본문+실시간 미리보기) + 메타바(v버전·작성/수정/승인자·일시). **버전관리**: 저장 시 이전 상태 스냅샷→versionHistory + version++, 기록 모달에서 보기/복원. **승인**: 승인 액션(승인자·시각 기록), 내용 수정 시 draft로 복귀(재승인). 별도 `canon_docs` 테이블 + rt-canon 실시간. switchView('canon') 3번째 뷰.
  - ⚠️ **필요 SQL(1회)**: `docs/tda_v40_supabase_migration.sql` §6 `canon_docs` 테이블 실행(미실행 시 정전 저장 실패 토스트). 그 외 기능은 graceful(테이블 없으면 빈 목록).

- **B2 결정/리뷰 컨텍스트화 + 직관화**: 칸반 카드 우클릭 → 컨텍스트 메뉴(결정 요청·문서 연결·처내기·삭제), 위키 문서 트리 우클릭 메뉴에 "결정 요청" 추가(기존 tree-context-menu 재사용). 리뷰 인박스에 기능 설명 문구(투표→ADR 자동생성, 우클릭 안내). 컨텍스트 메뉴 웜 테마 적용. → **B1/B2/B3 모두 완료.**
