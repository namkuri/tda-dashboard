...
3단계 (근거 추가): 의미상 더 풀 가치가 있는 곳에만 **세부 노드(3레벨) 2~6개**를
  핵심 데이터·예시·근거·인용·정의·반례·표 항목 등으로 추가합니다.
  (모든 서브가지에 3레벨을 넣을 필요 없음. 짧게 다룬 부분은 2레벨에서 끝.)

[작성 규칙]
1. **계층**: 최대 **3단계**(필요시 4단계까지 허용). 균일하게 만들지 마세요 — 풍부한 가지는 많이,
   단순한 가지는 적게. 마지막 계층을 기계적으로 2개로 끊지 마세요.
...
3단계 (세부): 의미 있는 곳에만 3레벨 2~6개로 데이터·예시·근거 추가.
4단계 (관계 명시): 서로 다른 문서/가지 사이의 의미 있는 연결을 cross_links에 기록.

[작성 규칙]
1. **계층**: 최대 3단계(필요시 4단계). 균일성 강요 금지 — 풍부한 가지 많이, 단순한 가지 적게.
